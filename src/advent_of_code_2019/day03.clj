...
(defn to-coordinates [start move]
  (let [direction (subs move 0 1)
        length (Integer/parseInt (subs move 1))
        coordinates (case direction
                      "L" (move-left start length)
                      "R" (move-right start length)
                      "U" (move-up start length)
                      "D" (move-down start length)
                      )]
    coordinates)
  )

(defn line-to-coordinates [start line coordinates]
  (if (empty? line)
    coordinates
    (let [[next-move & remaing-moves] line
          move-coordinates (to-coordinates start next-move)]
      (line-to-coordinates (last move-coordinates) remaing-moves (concat coordinates move-coordinates))
      )
    )
  )
